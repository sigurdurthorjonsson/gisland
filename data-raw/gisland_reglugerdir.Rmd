...
```{r sessionInfo, results='asis', echo=FALSE, eval=FALSE}
devtools::session_info()
```

This is a rudimentary draft text.

```{r x, comment='', echo=FALSE}
...
This document describes the steps taken to generate the object `XXX`

```{r some_constants}
cn <- c("id","type","year","nr","grein","tolulidur","undirlidur","heiti","dags_fra",
                "dags_til","v1","v2","v3","v5","v6","v7","v8","v9","v14","title","ref")

#PRO <- sp::CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
```
...
# Reglugerðir um takmörkun veiða

Read in all the data on reglugerdir:
```{r read_all_reglugerdir}
...
rg <- unique(dat[,c("rg","title","ref")])
```

---

### [2002_0162 um bann við veiðum með fiskibotnvörpu suðvestur af Malarrifi](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/aa0d47377abc977400256a090053ff91/b075510aa14af70b00256b6f003ec9f5?OpenDocument)
...
```{r r2002_0162, results='asis'}
year <- 2002
nr <- 162

x <- dat[dat$year %in% year & dat$nr %in% nr,]
datatable(prettyCoords(x[,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
...
leaflet() %>%
  addTiles(attribution = 'Data source: <a href="http://www.hafro.is">Marine Rearch Institute</a>') %>%
  addPolygons(data = tmp.spdf[i,], 
              weight=1, 
              group = "Reglugerðarlokanir", 
              col="red",
              popup = content)
```
...
### [2003_0068 Reglugerð um bann við veiðum með línu og fiskibotnvörpu á Sléttugrunni](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/58b439f05a7f412f00256a07003476bc/a5afb93a22d9571900256cc40055cb31?OpenDocument)
...
### [2003_0230 Reglugerð um bann við línuveiðum á Kötlugrunni](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/aa0d47377abc977400256a090053ff91/ccfd74b91f97fac100256cfb004af2f2?OpenDocument)

1. gr. Allar línuveiðar eru bannaðar á svæði á Kötlugrunni sem markast af línum sem dregnar eru milli eftirgreindra punkta:
```{r r2003_0230, results='asis'}
...
---
### [2003_0311 Reglugerð um bann við línuveiðum við Suður- og Suðausturland](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/lookByNumer/3112003?OpenDocument)
...
```{r r2003_0887, results='asis'}
# File <- paste(pck.path,"extdata/Reglugerd_2004_0202.csv",sep="/")
year <- 2003
nr <- 311
x <- dat[dat$year %in% year & dat$nr %in% nr,]
id <- 1
datatable(prettyCoords(x[x$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
          options = list(pageLength = 100, dom = 'tip'))
...
---
### [2004_0202 Reglugerð um um friðun blálöngu á hrygningartíma](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/58b439f05a7f412f00256a07003476bc/255d64822a6fa20900256e4e005b0cd2?OpenDocument)
...
---
### [2005_1140 Reglugerð um verndun kóralsvæða við suðurströndina](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/58b439f05a7f412f00256a07003476bc/2c53bb48f07a3c5f002570eb0052a6d0?OpenDocument)
...
---
### [2006_0050 Auglýsing um friðland í Surtsey](http://stjornartidindi.is/Advert.aspx?ID=611b81d2-d439-4c98-a21e-23b8bb9abcf8)
...
---
### [2006_861 Reglugerð um bann við veiðum með fiskibotnvörpu norðvestur af Heimaey](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/58b439f05a7f412f00256a07003476bc/997fd68542e3d6550025720a0049d17d?OpenDocument)
...
---
### [2007_0193 Reglugerð um bann við línuveiðum á norðanverðum Breiðafirði](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/aa0d47377abc977400256a090053ff91/5f7766f19e0bccfc0025729c00503de5?OpenDocument)
...
datatable(prettyCoords(x10[x10$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
          options = list(pageLength = 100, dom = 'tip'))
```
...
datatable(prettyCoords(x2[,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
          options = list(pageLength = 100, dom = 'tip'))
```
...
j <- which(tmp.spdf$nr == nr & tmp.spdf$type %in% "temporary")

content_i <- paste("<b><a href='",
                datDF$ref[i],
                "' target='_blank'",
                ">",
                datDF$title[i],
...
---
### [2007_0310 Reglugerð um friðunarsvæði við Ísland](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/2d8cdab9a540c73600256a0d0055eeb1/24d05c44a983604e002572c30030a382?OpenDocument)

```{r r2007_0310a, echo=FALSE}
year <- 2007
nr <- 310
x <- dat[dat$year %in% year & dat$nr %in% nr,]
```
...
```{r 2007_0310_1_A, results='asis', echo=FALSE}
id <- 2
datatable(prettyCoords(x[x$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
...
```{r 2007_0310_1_B, results='asis', echo=FALSE}
id <- 3
datatable(prettyCoords(x[x$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
...
```{r r2007_0310_1_2, echo=FALSE, results='asis'}
id <- 4
datatable(prettyCoords(x[x$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
...
```{r r2007_0310_1_3, echo=FALSE, results='asis'}
id <- 5
datatable(prettyCoords(x[x$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
...
```{r r2007_0310_1_3_manipulation}
# must be an easier way:
x1 <- x[x$id %in% id,]
x1$lat[5] <- x1$lat[1]
x1 <- rbind(x1,x1[1,])
x1$lon <- -geo_convert(x1$lengd)
x1$lat <-  geo_convert(x1$breidd)
...
```{r r2007_0310_2_1, echo=FALSE, results='asis'}
id <- 6
datatable(prettyCoords(x[x$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
...
```{r r2007_0310_2_2, echo=FALSE, results='asis'}
id <- 7
datatable(prettyCoords(x[x$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
...
```{r r2007_0310_2_3, echo=FALSE, results='asis'}
id <- 8
datatable(prettyCoords(x[x$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
...
```{r r2007_0310_2_4, echo=FALSE, results='asis'}
id <- 9
datatable(prettyCoords(x[x$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
...
```{r r2007_0310_2_5, echo=FALSE, results='asis'}
...
```{r r2007_0310_2_6, echo=FALSE, results='asis'}
id <- 11
datatable(prettyCoords(x[x$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
...
```{r r2007_0310_2_7, echo=FALSE, results='asis'}
id <- 12
datatable(prettyCoords(x[x$id == id,c("breidd","lengd")]),
          filter="none",
          rownames = FALSE,
...
---
### [Reglugerð 2007_0883 um breytingu á reglugerð nr. 310, 4. apríl 2007, um friðunarsvæði við Ísland](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/58b439f05a7f412f00256a07003476bc/a2e611ef8b30cef40025736e005ba1ac?OpenDocument)
...
```{r r2007_0883, echo=FALSE, results='asis'}
year <- 2007
nr <- 883
...
---
### [Reglugerð 2007_1041 breytingu á reglugerð nr. 310, 4. apríl 2007, um friðunarsvæði við Ísland](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/58b439f05a7f412f00256a07003476bc/c27298f0dbe9a0c50025738e00418450?OpenDocument)
...
```{r r2007_1041, echo=FALSE, results='asis'}
year <- 2007
nr <- 1041
x <- dat[dat$year == year & dat$nr == nr,]
id <- 1
...
---
### [Reglugerð 2008_0100 um bann við veiðum með fiskibotnvörpu í Eyrarbakkabug](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/58b439f05a7f412f00256a07003476bc/a9523a35bee8a67c002573e60047edc7?OpenDocument)
...
```{r r2008_0100_1, echo=FALSE, results='asis'}
year <- 2008
nr <- 100
x <- dat[dat$year == year & dat$nr == nr,]
id <- 1
...
---
#### [Reglugerð 2008_0752 um breytingu á reglugerð nr. 310, 4. apríl 2007, um friðunarsvæði við Íslands](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/58b439f05a7f412f00256a07003476bc/dc09acf8072cf9b7002574a2004fbf87?OpenDocument)
1. gr.

2. tl. 2. gr. orðist svo: Norðaustur af Horni á svæði, er markast af línum sem dregnar eru milli eftirgreindra punkta:
```{r r2008_0752, echo=FALSE, results='asis'}
year <- 2008
nr <- 752
x <- dat[dat$year == year & dat$nr == nr,]
id <- 1
...
---
### [Reglugerð 2009_0742 um bann við línu- og handfæraveiðum á Fljótagrunni](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/aa0d47377abc977400256a090053ff91/6e1741e608ec20d000257625003e9b08?OpenDocument)
...
```{r r2009_0742_1, echo=FALSE, results='asis'}
year <- 2009
nr <- 742
x <- dat[dat$year == year & dat$nr == nr,]
id <- 1
...
---
### [Reglugerð 2009_0810 um bann við línuveiðum á sunnanverðum Breiðafirði](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/aa0d47377abc977400256a090053ff91/f582cd06865034e900257646004d61e8?OpenDocument)

```{r r2009_0810_1, echo=FALSE, results='asis'}
year <- 2009
nr <- 810
x <- dat[dat$year == year & dat$nr == nr,]
id <- 1
...
---
#### [Reglugerð 2009_0887 um bann við línuveiðum fyrir Suðausturlandi](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/aa0d47377abc977400256a090053ff91/fedf5c5453c7f6ba0025766a006a1116?OpenDocument)
...
---
### [Reglugerð 2010_0520 um bann við veiðum með fiskibotnvörpu á Álseyjarbleyðu](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/58b439f05a7f412f00256a07003476bc/4d4bec1581eb9b250025774a0056cd8a?OpenDocument)
...
```{r r2010_0520_1, echo=FALSE, results='asis'}
year <- 2010
nr <- 520
x <- dat[dat$year == year & dat$nr == nr,]
id <- 20
...
---
### [Reglugerð 2011_1095 um verndun kóralsvæða](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/58b439f05a7f412f00256a07003476bc/7bc1628534a2c1fd0025795f003f67c0?OpenDocument)
...
---
### [2012_0034 Reglugerð um bann við veiðum með fiskibotnvörpu á Rauða torgi](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/58b439f05a7f412f00256a07003476bc/74cbd4e2cbb9a76d0025799d007c692e?OpenDocument)
...
```{r r2012_0034_1, echo=FALSE, results='asis'}
year <- 2012
nr <- 34
x <- dat[dat$year == year & dat$nr == nr,]
id <- 1
...
---
### [2013_0431 Reglugerð um stjórn veiða á djúpsjávartegundum á samningssvæði Norðaustur-Atlantshafsfiskveiðinefndarinnar (NEAFC)](http://www.reglugerd.is/reglugerdir/eftir-raduneytum/sjavaroglandbunadar/nr/18614)
...
---
### [2013_1065 Reglugerð um friðun veiðisvæða fyrir Suðurlandi](http://www.reglugerd.is/interpro/dkm/WebGuard.nsf/aa0d47377abc977400256a090053ff91/0cdeeb25919a30ae00257c36003b5c13?OpenDocument)
