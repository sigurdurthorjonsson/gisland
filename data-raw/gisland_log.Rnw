...
gpoints.df$long <- -geo_convert(gpoints.df$lengd)
gpoints.df$lat  <-  geo_convert(gpoints.df$breidd)
...
grunnlina_simple <- SpatialLines(list(po1b), proj4string = PRO)
grunnlina_simple <- SpatialLinesDataFrame(grunnlina_simple,data.frame(ID="1"))
@

<<eval=FALSE>>=
devtools::use_data(grunnlina)
devtools::use_data(grunnlina_simple)
@
...
<<eval=FALSE>>=
source("data-raw/drop_hole.R")
landhelgi_sldf <- drop_hole(landhelgi)
landhelgi_sldf <- as(landhelgi_sldf, "SpatialLinesDataFrame")
# Need to put each Line up to the root
l1 <- landhelgi_sldf@lines[[1]]@Lines[[1]]
l1 <- Lines(l1, ID="1")
l2 <- landhelgi_sldf@lines[[1]]@Lines[[2]]
l2 <- Lines(l2, ID="2")
l <- SpatialLines(LinesList = list(l1,l2), proj4string = PRO)
landhelgi_sldf <- SpatialLinesDataFrame(l,data.frame(ID=c("1","2")))
library(leaflet) 
leaflet() %>% addTiles() %>% addPolylines(data=landhelgi_sldf)
devtools::use_data(landhelgi_sldf)
@
...
<<eval=FALSE>>=
eez_sldf <- drop_hole(eez)
eez_sldf <- as(eez_sldf, "SpatialLinesDataFrame")
devtools::use_data(eez_sldf)
@
...
vidmidunarlina_simple <- Lines(list(l1), ID = "1")
vidmidunarlina_simple <- SpatialLines(list(vidmidunarlina_simple), proj4string = PRO)
vidmidunarlina_simple <- SpatialLinesDataFrame(vidmidunarlina_simple, data.frame(ID = "1"))
# devtools::use_data(vidmidunarlina_simple)
